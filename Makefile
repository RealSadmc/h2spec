...
build-container:
	docker build --build-arg VERSION=$(VERSION) --build-arg COMMIT=$(COMMIT) -t realsadmc/h2spec:latest -t realsadmc/h2spec:$(VERSION) .

push-container:
	docker push realsadmc/h2spec:latest

push-release-container:
	docker push realsadmc/h2spec:$(VERSION)
